...
class Franka(base.Task):
    ...
    def initialize_episode(self, physics):
        ...
        physics.reset()
        if self.task == 'reach':
            physics.named.model.site_pos['target_ball', 0] = 0.5
            physics.named.model.site_pos['target_ball', 1] = 0
            physics.named.model.site_pos['target_ball', 2] = 0.75
        elif self.task == 'push':
            ...
            physics.named.model.site_pos['target_box', 0] = 0.775
            physics.named.model.site_pos['target_box', 1] = 0
            physics.named.model.body_pos['object_box', 0] = 0.505
            physics.named.model.body_pos['object_box', 1] = 0
    ...
    def get_reward(self, physics):
        ...
        if self.task == 'reach':
            ...
            return rewards.tolerance(distance, bounds=(0, 0.01), margin=0.2) - 0.01 * action_penalty
        elif self.task == 'push':
            lfinger_to_target = physics.lfinger_to_target()
            lfinger_to_target_reward = rewards.tolerance(lfinger_to_target, bounds=(0, 0.001), margin=0.2, value_at_margin=0.3)
            rfinger_to_target = physics.rfinger_to_target()
            rfinger_to_target_reward = rewards.tolerance(rfinger_to_target, bounds=(0, 0.001), margin=0.2, value_at_margin=0.3)
            object_to_otarget = physics.object_to_otarget()
            object_to_otarget_reward = rewards.tolerance(object_to_otarget, bounds=(0, 0.01), margin=0.2, value_at_margin=0.3)
